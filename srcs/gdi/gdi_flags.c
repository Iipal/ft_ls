...
/*   Updated: 2020/01/25 19:33:03 by tmaluh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void
	gdif_internal_checks(const int32_t mask)
{
...
inline void
	gdi_flags_parser(struct s_arg_av *restrict a)
{
	...
	while (!stop && a->ac && '-' == **a->av)
	{
		while (!stop && *++*a->av)
			...
			> (flag_i = ft_strchr(valid_flags, **a->av) - valid_flags))
				gdif_internal_checks(GDI_FLAGS_SET_BIT(TO_N_BIT(flag_i)));
			else
				errx(EXIT_FAILURE,
					"illegal option -- %c\n%s", **a->av, FLAGS_USAGE);
